...
MAESTRO takes a different approach: instead of asking one model to be everything, we give each role to a specialist. The orchestrator understands the task and creates a plan. The coder writes the implementation. The reviewer checks for issues. The tester verifies it works. Each agent has a focused context window with only what it needs.

## Smart Model Routing — Multi-Agent Without Multi-Dollar

The obvious concern with multi-agent is cost. MAESTRO solves this with smart model routing: the orchestrator runs a capable model (e.g. Claude Sonnet) that needs to understand architecture and make decisions, while sub-agents run faster, cheaper models (e.g. Haiku) for focused tasks like writing a function or running a test.

The result: you get a team at roughly the cost of a single generalist agent.

## Context Engineering — The Hidden Superpower

The real bottleneck in agentic coding isn't model intelligence — it's context management. MAESTRO implements:
...
One command. Set your API key in `.env`. Everything else in `maestro.yaml`. Done.
...
| Smart model routing | Expensive models for decisions, cheap models for execution |
